...
*   **Board-Level Hook (`on_config`):** The `cfn_hal_driver_t` base struct contains an `on_config` callback. The generic `_init` and `_deinit` wrappers automatically call this hook before/after touching hardware registers. It uses a `cfn_hal_config_phase_t` enum to distinguish between initialization and deinitialization phases.
...
*   **Generic Callbacks:** Generic callbacks in the base layer must use `cfn_hal_callback_t` (function pointer) instead of `void*` to satisfy strict C11 conversion rules. Peripheral wrappers should cast their specific function pointers to `cfn_hal_callback_t` when calling base functions.
...
*   **Layered FourCC Identification:** Use `CFN_HAL_PERIPHERAL_PREFIX` (defined as 'A') for all HAL peripheral types to allow for multi-layered identification.
*   **Naming Conventions:** 
...
*   **Locking Policy:** Static inline functions should **not** call `CFN_HAL_LOCK` internally. Concurrency protection is the responsibility of the caller using the `CFN_HAL_WITH_LOCK` macro, which utilizes the `cfn_hal_base_lock` and `cfn_hal_base_unlock` functions.
